...
struct
  ...
  fun exec (v :: S, _, nil, nil) = v
    ...
    | exec (BOOL false :: S, E, If (C1, C2) :: C, D) =
        exec (S, E, C2 @ C, D)
    | exec (_, _, C, _) =
        (print (codeToString C); raise RuntimeError)
  fun run env (id, code) =
    let
