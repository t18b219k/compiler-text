...
struct
  ...
  in
    ...
    fun substTy subst ty =
      ...
      | TYVARty string =>
          (case SEnv.find (subst, string) of
             NONE => ty
           | SOME ty => ty)
      | FUNty (ty1, ty2) => FUNty (substTy subst ty1, substTy subst ty2)
      | PAIRty (ty1, ty2) => PAIRty (substTy subst ty1, substTy subst ty2)
      | POLYty (tids, ty) => POLYty (tids, substTy subst ty)
    fun composeSubst subst1 subst2 =
      SEnv.unionWith (fn (ty1, ty2) => ty1)
    ...
    fun unionTyEnv (tyEnv1, tyEnv2) = SEnv.unionWith #1 (tyEnv1, tyEnv2)
    fun removeTyEnv (tyEnv, string) =
      ...
      case ty of
        POLYty (tids, ty) =>
          let
            val S =
              foldr
                (fn (tid, S) => let val newty = newTy ()
                                in SEnv.insert (S, tid, newty)
                                end) emptySubst tids
          in
            substTy S ty
          end
