...
structure Typeinf = struct
  ...
  exception IfArmsNotMatch of {then_ty:ty,else_ty:ty,then_exp:exp,else_exp:exp}
  exception TypeError
  ...
  fun typed_W gamma exp =
    ...
    | EXPAPP (exp1, exp2) =>
        let
          ...
          val ty3 = newTy ()
          val S3 = (unify [(FUNty (ty2, ty3), substTy S2 ty1)] )handle UnifyTy(a,b,_) =>
           case b of 
           FUNty(actual,result)=>
           if actual = ty2 then 
            raise ResultNotMatch{expected = ty3,actual = result,expression=exp1 }
           else 
            raise ArgNotMatch{expected = ty2,actual = actual,expression=exp2}
          |_=>raise NotFunction {expected = a,actual = b,expression = exp1 } 
    ...
    | EXPPROJ1 exp =>
        let
          ...
          val ty1 = newTy ()
          val ty2 = newTy ()
          val S2 = unify [(ty, PAIRty (ty1, ty2))] handle UnifyTy(actual,expected,_)=>raise ExpectedToBePair{expected=expected,actual=actual, expression = exp}
        in
    ...
    | EXPIF (exp1, exp2, exp3) =>
        let
          val (S1, ty1, typed_exp1) = typed_W gamma exp1
          val S2 = unify [(ty1, BOOLty)]handle UnifyTy(a,b,_)=>raise ExpectedToBeBool{actual = a, expression = exp1}
          val (S3, ty2, typed_exp2) = typed_W (substTyEnv (S2 ++ S1) gamma) exp2
          val (S4, ty3, typed_exp3) = typed_W (substTyEnv (S3 ++ S2 ++ S1) gamma) exp3
          val S5 = unify [(ty2, ty3)] handle UnifyTy(a,b,_)=>raise IfArmsNotMatch {then_ty= a,then_exp=exp2,else_ty = b ,else_exp=exp3}
    ...
    | EXPPRIM (p, exp1, exp2) =>
        let
          val (S1, ty1, typed_exp1) = typed_W gamma exp1
          val (S2, ty2, typed_exp2) = typed_W (substTyEnv S1 gamma) exp2
          val S3 = unify [(substTy S2 ty1,INTty),(ty2,INTty)] handle UnifyTy(a,b,id)=> 
          case id of 
