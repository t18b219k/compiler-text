...
struct
  ...
  fun print_error (s, pos1, pos2) =
    print
      ("Syntax error(" ^ Int.toString pos1 ^ "-" ^ Int.toString pos2 ^ ") :" ^ s
       ^ "\n")
  fun discardSemicolons stream =
    let
      val (token, rest) = P.getStream stream
    in
      if P.sameToken (token, T.SEMICOLON (0, 0)) then discardSemicolons rest
  ...
  fun doParse stream =
    let
      ...
      val (dec, stream) = P.parse
        {lookahead = 0, stream = stream, error = print_error, arg = ()}
