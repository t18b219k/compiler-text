...
struct
  datatype PPContext = context of {nest_level: int}

  fun generate_tabs (context {nest_level = 0}) = ""
    | generate_tabs (context {nest_level}) =
        "\t" ^ generate_tabs (context {nest_level = nest_level - 1})
  ...
  fun paramToString (param (id, ty)) =
    "(param $" ^ id ^ " " ^ valtypeToString ty ^ ")"

  datatype result = result of valtype

  fun resultToString (result ty) =
    "(result " ^ valtypeToString ty ^ ")"

  datatype functype = functype of param list * result list

  fun functypeToString (functype ([], [])) = "(func)"
    | functypeToString (functype (xs, [])) =
        "(func " ^ foldr (fn (x, K) => paramToString x ^ K) "" xs ^ ")"
    | functypeToString (functype ([], xs)) =
        "(func " ^ foldr (fn (x, K) => resultToString x ^ K) "" xs ^ ")"
    | functypeToString (functype (params, results)) =
        "(func " ^ (foldr (fn (x, K) => paramToString x ^ K) "" params) ^ " "
        ^ (foldr (fn (x, K) => resultToString x ^ K) " " results ^ ")")
  ...
  fun tabletypeToString (tabletype (l, rt)) =
    limitsToString l ^ " " ^ reftypeToString rt
  ...
  fun memargToString (memarg (offset, align)) =
    "offset=" ^ Word32.toString offset ^ " " ^ "align=" ^ Word32.toString align
  ...
  fun type_definitionToString (type_definition (id, ft)) =
    "(type "
    ^
    (case id of
       SOME id => "$" ^ id
  ...
  fun typeuseToString ty_use =
    ...
    | with_functype (id, params, results) =>
        "(type " ^ IDX.typeidxToString id ^ ")"
  ...
  fun instructionToString inst =
    case inst of
      block_i (l, bt, iseq) =>
        ...
        ^ foldr (op^) "" (map instructionToString iseq) ^ "end"
    | loop (l, bt, iseq) =>
        "loop "
        ^
        (case l of
    ...
    | if_ (l, bt, iseq_true, iseq_false) =>
        ...
        foldr (fn (inst, buf) => inst ^ "\n" ^ buf) ""
          (map instructionToString iseq_true) ^ "else\n"
        ^
        foldr (fn (inst, buf) => inst ^ "\n" ^ buf) ""
          (map instructionToString iseq_false) ^ "end"
    | nop => "nop"
    | unreachable => "unreachable"
    | br l => "br " ^ IDX.labelidxToString l
    | br_if l => "br_if " ^ IDX.labelidxToString l
    | br_table (label_list, l) =>
        "br_table "
        ^
        foldr (fn (l, buf) => (IDX.labelidxToString l) ^ "\n" ^ buf) ""
          label_list ^ " " ^ IDX.labelidxToString l
    | return => "return"
    | call funcidx => "call " ^ IDX.funcidxToString funcidx
    | call_indirect (tableidx, typeuse) =>
        "call_indirect " ^ IDX.tableidxToString tableidx ^ " "
        ^ typeuseToString typeuse
    | drop => "drop"
    | select => "select"
    ...
    | tablecopy (tableidx1, tableidx2) =>
        "table.copy " ^ IDX.tableidxToString tableidx1 ^ " "
        ^ IDX.tableidxToString tableidx2
    | tableinit (tableidx, elemidx) =>
        "table.init " ^ IDX.tableidxToString tableidx ^ " "
        ^ IDX.elemidxToString elemidx
  ...
  and pp_expr_bracketted ctx [inst] = pp_instruction ctx inst
    | pp_expr_bracketted ctx il =
        foldr (fn (x, K) => pp_instruction ctx x ^ "\n" ^ K) "" il

  (*| x => raise DumpInstruction x*)
  type expr = instruction list

  fun exprToString_bracketted [inst] = instructionToString inst
    | exprToString_bracketted il =
        foldr (fn (x, K) => instructionToString x ^ "\n" ^ K) "" il

  fun exprToString expr = exprToString_bracketted expr ^ "\n end"
  ...
  fun importToString (import (module, nm, d)) =
    "(import \"" ^ module ^ "\" \"" ^ nm ^ "\" " ^ import_descToString d ^ ")"

  datatype local_ = local_ of IDX.localidx option * valtype

  fun local_ToString (local_ (id, vt)) =
    "(local "
    ^
    (case id of
       NONE => ""
  ...
  fun exportToString (export (name, desc)) =
    "(export \"" ^ name ^ "\" " ^ export_descToString desc ^ ")"

  datatype start = start of IDX.funcidx

  fun startToString (start fid) =
    "(start " ^ IDX.funcidxToString fid ^ ")"

  fun pp_start ctx (start fid) =
    generate_tabs ctx ^ "(start " ^ IDX.funcidxToString fid ^ ")"

  datatype elemexpr = elemexpr of expr

  fun elemexprToString (elemexpr e) =
    "(item " ^ exprToString_bracketted e ^ ")"

  datatype elemlist = elemlist of reftype * elemexpr list

  fun elemlistToString (elemlist (rt, eel)) =
    reftypeToString rt ^ " "
    ^ (foldr (fn (x, xs) => elemexprToString x ^ "\n" ^ xs) "" eel)
  ...
  fun elemmodeToString em =
    case em of
      passive_elem_mode => ""
    | active_elem_mode (tid, e) =>
        "(table " ^ IDX.tableidxToString tid ^ ")" ^ "(offset "
        ^ exprToString_bracketted e ^ ")"
    | declarative => "declare"

  datatype elem = element of IDX.elemidx option * elemlist * elemmode

  fun elementToString (element (id, el, em)) =
    "(elem "
    ^
    (case id of
       NONE => ""
     | SOME id => IDX.elemidxToString id) ^ elemmodeToString em
    ^ elemlistToString el ^ ")"
  ...
  fun datamodeToString dm =
    case dm of
      passive_data_mode => ""
    | active_data_mode (id, e) =>
        IDX.memidxToString id ^ "(offset " ^ exprToString_bracketted e ^ ")"
  ...
  *)
  datatype data =
    data of IDX.dataidx option * datamode * string

  fun dataToString (data (did, dm, s)) =
    "(data "
    ^
    (case did of
       NONE => ""
  ...
  fun typesToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map type_definitionToString tl)

  fun pp_types ctx tl =
    foldr
      (fn (x, K) => (generate_tabs ctx ^ type_definitionToString x ^ "\n" ^ K))
      "" tl

  type funcs = func list

  fun funcsToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map funcToString tl)

  fun pp_funcs ctx funcs =
    foldr (fn (x, K) => (generate_tabs ctx ^ pp_func ctx x ^ "\n" ^ K)) "" funcs

  type tables = table list

  fun tablesToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map tableToString tl)

  fun pp_tables ctx tables =
    foldr (fn (x, K) => generate_tabs ctx ^ tableToString x ^ "\n" ^ K) ""
      tables
  ...
  fun pp_mems ctx mems =
    foldr (fn (x, K) => generate_tabs ctx ^ memToString x ^ "\n" ^ K) "" mems

  type globals = global list

  fun globalsToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map globalToString tl)

  fun pp_globals ctx globals =
    (foldr (fn (x, K) => generate_tabs ctx ^ globalToString x ^ "\n" ^ K) ""
       globals)

  type elems = elem list

  fun elemsToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map elementToString tl)

  fun pp_elems ctx elems =
    foldr (fn (x, K) => generate_tabs ctx ^ elementToString x ^ "\n" ^ K) ""
      elems

  type datas = data list

  fun datasToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map dataToString tl)

  fun pp_datas ctx datas =
    foldr (fn (x, K) => generate_tabs ctx ^ dataToString x ^ "\n" ^ K) "" datas

  type imports = import list

  fun importsToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map importToString tl)

  fun pp_imports ctx imports =
    foldr (fn (x, K) => generate_tabs ctx ^ importToString x ^ "\n" ^ K) ""
      imports

  type exports = export list

  fun exportsToString tl =
    foldr (fn (x, xs) => (x ^ "\n" ^ xs)) "" (map exportToString tl)

  fun pp_exports ctx exports =
    foldr (fn (x, K) => generate_tabs ctx ^ exportToString x ^ "\n" ^ K) ""
      exports
  ...
  fun pp_module module =
    ...
    in
      "(module\n" ^ types ^ imports ^ functions ^ tables ^ mems ^ globals
      ^ exports ^ start_segment ^ elems ^ datas ^ ")"
    end

  fun moduleToString module = pp_module module

  val emptyModule =
    { ty = [type_definition (SOME "entry_point", functype ([], []))]
    , fn_ = []
    , ta = []
    , me = []
    , gl = []
    ...
    , im = []
    , ex = []
    , st = start (IDX.funcidx (IDX.text_id "__cml_main"))
    }
end
